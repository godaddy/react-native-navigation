...
-(void)onButtonPress:(UIButton*)buttonItem
{
  NSString *callbackId = objc_getAssociatedObject(buttonItem, &CALLBACK_ASSOCIATED_KEY);
  if (!callbackId) return;
  NSString *buttonId = objc_getAssociatedObject(buttonItem, &CALLBACK_ASSOCIATED_ID);
...
-(void)onBarButtonPress:(UIBarButtonItem*)barButtonItem
{
...
<<<<<<< HEAD
      ...
      if (buttonId)
      {
        objc_setAssociatedObject(buttonItem, &CALLBACK_ASSOCIATED_ID, buttonId, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
      }
    } else {
      if (iconImage)
      {
